...
fun TeamEdit(
    teamId: Int,
    viewModel: TeamEditViewModel = koinViewModel(),
) {
    ...
    LaunchedEffect(key1 = team?.nameTeam) {
        if (!team?.nameTeam.isNullOrEmpty()) {
            teamName = team?.nameTeam ?: "Tanpa Nama"
        }
    }
    LaunchedEffect(key1 = team?.description) {
        if (!team?.nameTeam.isNullOrEmpty()) {
            teamDescription = team?.description ?: "Tanpa Deskripsi"
        }
    }
    Column(
        modifier = Modifier
        ...
        Button(
            ...
            modifier = Modifier
                ...
                .padding(top = 8.dp),
            onClick = {
                viewModel.updateTeam(teamId, teamName, teamDescription)
            },
        ) {
            Text(
                text = "Edit Tim"
